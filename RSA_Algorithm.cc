...
void RSA_Algorithm::primeGenerator(mpz_class& num, int sz)
{
    
    while(1)
    {
        num = rng.get_z_bits(sz);
        if(mpz_probab_prime_p(num.get_mpz_t(), 100) == 1)
        {
            return;
        }
    }
...
mpz_class RSA_Algorithm::Encrypt(mpz_class M){
    mpz_class output;
    mpz_powm(output.get_mpz_t(), M.get_mpz_t(), e.get_mpz_t(), n.get_mpz_t());
    return output;
}

mpz_class RSA_Algorithm::Decrypt(mpz_class C){
    mpz_class output;
    mpz_powm(output.get_mpz_t(), C.get_mpz_t(), d.get_mpz_t(), n.get_mpz_t());
    return output;
}
